...
df_bond = stockBondsDataFromDD(bond, period, end_day)
...
if re.match(rep, ticker):
    ...
    fig.add_trace(go.Scatter(x=df_bond.index, y=df_bond[f"DGS{bond}"], name=f"米国債券{bond}年利回り", yaxis="y2",
    ...
    fig.update_layout(width=1200, height=800, xaxis_rangeslider_visible=False, font_size=20,
                      ...
                      yaxis2=dict(side="right", title=f"米国債券{bond}年利回り", showgrid=False, overlaying="y"))
